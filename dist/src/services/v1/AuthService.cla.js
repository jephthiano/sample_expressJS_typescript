...
class AuthService {
    ...
    static async login(req) {
        ...
        if (!user)
            triggerError("Incorrect login details", [], 401);
    ...
    static async register(req) {
        ...
        if (!user)
            triggerError("Account creation failed", [], 500);
    ...
    static async sendOtp(req, type) {
        ...
        if (!sent)
            triggerError("Request for otp failed", [], 500);
        return [];
    }
    // [VERIFY OTP]
    ...
    static async resetPassword(req) {
        ...
        if (!updateUserData)
            triggerError("Password reset failed", [], 500);
    ...
    static async logout(req) {
        ...
        if (!response)
            triggerError("Request failed, try again", [], 500);
        return response;
    }
}
