...
async function transformUserUpdate(update) {
    ...
    if (target.email)
        target.email = selEncrypt(target.email.toLowerCase(), 'email');
    if (target.mobile_number)
        target.mobile_number = selEncrypt(target.mobile_number, 'mobile_number');
    if (target.username)
        target.username = selEncrypt(target.username.toLowerCase(), 'username');
    if (target.first_name)
        target.first_name = selEncrypt(target.first_name.toLowerCase(), 'first_name');
    if (target.last_name)
        target.last_name = selEncrypt(target.last_name.toLowerCase(), 'last_name');
    if (update.$set)
        update.$set = target;
    else
