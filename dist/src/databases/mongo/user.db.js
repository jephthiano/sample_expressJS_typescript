...
const findUserByEmailOrPhone = async (receiving_medium) => {
    ...
    return await User.findOne({ $or: [{ mobile_number: enc_receiving_medium }, { email: enc_receiving_medium }] });
};
...
const resetUserPaswword = async (data) => {
    ...
    const user = await User.findOneAndUpdate({ $or: [{ mobile_number: encMedium }, { email: encMedium }] }, { password }, { new: true });
    if (!user)
        return null;
    return {
...
export { findUserByID, findUserByEmailOrPhone, findEmailMobileNumberUsername, findUserSingleValue, findUserSingleValuebyEncField, createUserAccount, resetUserPaswword };
